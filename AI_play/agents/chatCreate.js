...
class ChatCreateAI extends AIbase {
    ...
    async start() {
        ...
        let result = await requestAI(`${post.content_preview}에 대한 글을 평가를 하거나 피드백을 하거나 질문을 짧게 해줘.`, {
            system: `당신의 이름은 ${user.name} 입니다. 당신은 글을 읽고 있는 독자 입니다. 글의 주인은 ${owner_name} 입니다. Dnote 사이트의 블로그 글을 보고 있습니다. 댓글 처럼 표현하세요. 당신의 나이는 ${age}살 입니다. 나이에 맞는 표현을 사용하세요. 한국어(Korean)으로 대답해야합니다.`
        });
