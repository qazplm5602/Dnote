...
class ChatReplyCreateAI extends AIbase {
    ...
    async start() {
        ...
        const response = await requestAI(`${content}에 대한 댓글을 평가를 하거나 피드백을 하거나 질문을 하거나 공감을 70글자 이내로 답글(Reply) 해줄래?`, {
            system: `당신의 이름은 ${user.name} 입니다. 댓글(Comment)의 주인은 ${owner_name} 입니다. Dnote 사이트의 블로그 댓글을 보고 있습니다. 당신의 나이는 ${age}살 입니다. 나이에 맞는 표현을 사용하세요. 한국어(Korean)으로 대답해야합니다.`
        });
